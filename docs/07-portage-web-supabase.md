...
_« local-first : aucune requête réseau au runtime, pas de compte »_ est **assoupli en option
...
- Le serveur n'a jamais besoin de _comprendre_ le contenu (le moteur recalcule client‑side) → une
...
**Justification.** Tu veux _un_ compte que tu définis. Pas de flux d'inscription = pas de surface
...
_même_ jour sur deux appareils dans la même fenêtre). LWW est suffisant, simple, déterministe et
...
_normal et prévu_). **Toute** la sécurité d'isolation repose donc sur RLS. Une policy ratée = données
...
- **Authentication → Sign‑ups** : _Disable new user signups_ (compte créé à la main).
...
- **MFA (recommandé)** : activer _TOTP_ dans `Authentication → MFA`.
- **URL Configuration** : ajouter `https://<user>.github.io/crohnos` aux _Redirect URLs_ et au
  _Site URL_ (sinon le flux de connexion web est rejeté).
- **Mot de passe** : longueur minimale ≥ 12, vérification _leaked password_ (HaveIBeenPwned) activée.
...
- **CI GitHub Actions** : les mêmes via _Repository secrets_ (pour le build web). La `service_role`
...
1. À la première activation, l'utilisateur définit une **passphrase de chiffrement** _distincte_ du
...
Le schéma de transport (§5.1) est inchangé : seul le _contenu_ de `contenu` passe de JSON clair à
...
### Phase 0 — Découplage de la persistance _(prérequis, sans réseau)_ ✅ LIVRÉE
...
- **Sortie :** mobile fonctionne identiquement, store testable sans émulateur. _Aucune dépendance
  Supabase encore._
...
### Phase 3 — Chiffrement de bout en bout _(recommandé)_ ✅ LIVRÉE

- [x] Activation E2EE : passphrase distincte, dérivation de clé (PBKDF2-SHA256, 150 k iters), clé
  gardée **en mémoire seule** (choix produit validé : aucune persistance disque, mobile comme web →
  re-saisie à chaque lancement, rien de déchiffrable ne traîne sur l'appareil).
- [x] Chiffrer/déchiffrer `contenu` via un **`CodecContenu`** injecté (réutilise `chiffrement.ts`,
  primitives clé-niveau `chiffrerOctets`/`dechiffrerOctets` → 1 dérivation, N chiffrements) : branché
  dans `depotSupabase.ts` (web) **et** `transportSupabase.ts` (sync mobile). Coffre runtime
  `coffreE2EE.ts` (`creerCodecCoffre`) ; logique pure `e2ee.ts` (testée hors réseau).
- [x] **Partage multi-appareils** (non spécifié au départ) : sel + canari (témoin chiffré) stockés
  côté cloud dans `MetaE2EE` (entité `e2ee`, en clair car non secrets, hors périmètre de sync). Tout
  appareil saisissant la bonne passphrase re-dérive la même clé et la valide via le canari.
- [x] Migration des données déjà en clair : `rechiffrerTout` (e2eeCloud.ts) ré-écrit l'existant à
  l'activation ; `dechiffrerContenu` reste rétrocompatible (passe-plat sur du clair). Meta publiée
  **avant** la migration → un état mixte clair/chiffré reste lisible (jamais d'opaque sans meta).
- [x] Garde de session : carte E2EE dans Réglages (activer/déverrouiller/état) + écran de
  déverrouillage web (`app/deverrouillage.tsx`, garde dans `_layout.tsx`) ; la sync mobile refuse de
  pousser en clair tant que l'E2EE configuré n'est pas déverrouillé.
- [x] Avertissement « passphrase perdue = données illisibles » affiché à l'activation et au verrou.
- **Sortie :** Supabase ne stocke plus que de l'opaque ; fidélité à l'esprit local‑first.
- **Hors lot (suivi) :** persistance de session Supabase mobile (limite connue Phase 2) — incrément
  séparé. La passphrase E2EE n'est volontairement jamais persistée (choix produit).
...
### 10.3 Sécurité RLS _(non négociable)_
